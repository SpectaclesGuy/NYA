...
async def create_request(payload: RequestCreate, current_user=Depends(require_onboarding_complete), db=Depends(get_db)):
...
async def incoming_requests(current_user=Depends(require_onboarding_complete), db=Depends(get_db)):
    service = RequestService(db)
    return await service.list_incoming(current_user["id"])


@router.get("/outgoing", response_model=list[RequestListItem])
async def outgoing_requests(current_user=Depends(require_onboarding_complete), db=Depends(get_db)):
    service = RequestService(db)
    return await service.list_outgoing(current_user["id"])


@router.post("/{request_id}/accept", response_model=RequestSummary)
async def accept_request(request_id: str, current_user=Depends(require_onboarding_complete), db=Depends(get_db)):
    service = RequestService(db)
...
async def reject_request(request_id: str, current_user=Depends(require_onboarding_complete), db=Depends(get_db)):
    service = RequestService(db)
